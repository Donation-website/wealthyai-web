...
export default function PremiumMonth() {
  ...
  /* ================= NEW EMAIL LOGIC (REPAIRED) ================= */

  const sendEmailPDF = () => {
    if (!activeText) return;
    // Megnyitjuk a modal ablakot ahelyett, hogy azonnal küldenénk
  ...
  const confirmAndSendEmail = async () => {
    ...
    try {
      const res = await fetch("/api/send-month-email", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          text: activeText, 
          cycleDay, 
          region,
          email: userEmail // Átadjuk a felhasználó által beírt címet
        }),
      });
      
      if (res.ok) {
        alert("Strategic briefing sent to " + userEmail);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={card}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 20 }}>
            ...
            <button onClick={runAI} style={{ ...aiButton, marginTop: 0 }}>{loading ? "Generating..." : "GENERATE AI"}</button>
          </div>
          <button onClick={runAIDual} style={{ ...exportBtn, marginTop: 12, width: "100%" }}>Save Today’s Snapshot</button>
        </div>
        ...
        <div style={card}>
          {!aiVisible && !simulationActive && (
            <div style={{ padding: "10px" }}>
              <strong style={{ color: "#10b981", fontSize: 12 }}>WEALTHYAI PHILOSOPHY</strong>
              <h2 style={{ fontSize: 22, marginTop: 10 }}>Interpretation, Not Advice.</h2>
              <p style={{ opacity: 0.7, fontSize: 14 }}>Our system assumes that you remain responsible for decisions.</p>
            </div>
          )}
          ...
          {simulationActive && !aiVisible && (
            <div style={{ padding: "10px" }}>
              ...
              <div style={{ fontSize: 42, fontWeight: "bold", color: "#38bdf8", margin: "15px 0" }}>{calculateFragility()}%</div>
              <button onClick={() => setSimulationActive(false)} style={{ ...exportBtn, opacity: 0.6 }}>Reset</button>
            </div>
          )}
          ...
          <Divider />
          <button onClick={() => setArchiveOpen(!archiveOpen)} style={{ ...exportBtn, width: "100%" }}>{archiveOpen ? "Hide past days" : "View past days"}</button>
          {archiveOpen && (
            <div style={{ marginTop: 10 }}>
              {getMonthlySnapshots().map(s => (
                <button key={s.date} onClick={() => { setSelectedDay(s.cycleDay); setAiVisible(true); setSimulationActive(false); }} style={{ ...exportBtn, marginBottom: 4, width: "100%" }}>Day {s.cycleDay}</button>
              ))}
            </div>
          )}
        </div>
      ...
      {/* ================= MODAL WINDOW (RE-ADDED) ================= */}
      {emailModalOpen && (
        <div style={modalOverlay}>
          <div style={{...card, maxWidth: 400, width: "90%", animation: "fadeIn 0.2s ease-out"}}>
            <h3 style={{marginTop: 0}}>Send Briefing</h3>
            <p style={{fontSize: 13, opacity: 0.7, marginBottom: 15}}>Where should we send your strategic report?</p>
            <input 
              type="email" 
              placeholder="Enter your email address"
              value={userEmail}
              onChange={(e) => setUserEmail(e.target.value)}
              style={{...input, marginBottom: 20, border: "1px solid #38bdf8", padding: "12px"}}
            />
            <div style={{display: "flex", gap: 10}}>
              <button onClick={() => setEmailModalOpen(false)} style={{...exportBtn, flex: 1}}>Cancel</button>
              <button onClick={confirmAndSendEmail} style={{...aiButton, flex: 2, marginTop: 0}}>
...
const page = {
  minHeight: "100vh", position: "relative", padding: "40px 20px",
  color: "#e5e7eb", fontFamily: "Inter, system-ui", backgroundColor: "#020617",
  backgroundImage: `radial-gradient(circle at 20% 30%, rgba(56,189,248,0.14), transparent 45%), radial-gradient(circle at 80% 60%, rgba(167,139,250,0.14), transparent 50%)`,
};
