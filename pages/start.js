...
export default function UserDashboard() {
  ...
  const usagePercent = data.income > 0 ? Math.min((totalExpenses / data.income) * 100, 100) : 0;
  ...
  const riskLevel = usagePercent > 90 ? "High Risk" : usagePercent > 70 ? "Medium Risk" : "Low Risk";

  /* ===== INSIGHTS ===== */
  const insights = [];
  if (balance < 0) insights.push("Your expenses exceed your income. Immediate action may be required.");
  if (data.subscriptions > data.income * 0.08) insights.push("Subscriptions appear high. Reviewing unused services may free up cash.");
  if (savingsRate >= 20) {
  ...
  /* ===== STRIPE (FIXED FOR TEST MODE) ===== */
  const handleCheckout = async (priceId) => {
    localStorage.setItem("userFinancials", JSON.stringify(data));

    // 🔐 UPDATED TO YOUR NEW TEST ID
  ...
  const radar = [
    ...
    { label: "Subscription Weight", value: data.income > 0 ? Math.min((data.subscriptions / data.income) * 200, 100) : 0 },
  ];
  ...
  const Radar = ({ data, size = isMobile ? 180 : 200 }) => {
    ...
    return (
      <svg width={size} height={size} style={{ display: "block", margin: "20px auto", overflow: "visible" }}>
        {[0.25, 0.5, 0.75, 1].map((lvl, i) => (
          <circle key={i} cx={c} cy={c} r={r * lvl} fill="none" stroke="rgba(255,255,255,0.12)" />
        ))}
        {data.map((_, i) => {
          const a = i * step - Math.PI / 2;
          return <line key={i} x1={c} y1={c} x2={c + r * Math.cos(a)} y2={c + r * Math.sin(a)} stroke="rgba(255,255,255,0.18)" />;
        })}
        <polygon points={data.map((d, i) => point(d.value, i).join(",")).join(" ")} fill="rgba(99,102,241,0.35)" stroke="rgba(99,102,241,0.9)" />
        {data.map((d, i) => {
          const a = i * step - Math.PI / 2;
          return (
            <text key={i} x={c + (r + 14) * Math.cos(a)} y={c + (r + 14) * Math.sin(a)} fontSize="11" fill="rgba(255,255,255,0.7)" textAnchor="middle" dominantBaseline="middle">
              {d.label}
            </text>
          );
        })}
  ...
  const input = {
    width: "100%", padding: "10px", marginTop: "6px", borderRadius: "8px", border: "none", background: "rgba(255,255,255,0.08)", color: "white", boxSizing: "border-box",
  };

  const priceCard = {
    ...card, textAlign: "center", cursor: "pointer", flex: isMobile ? "1 1 100%" : "0 1 240px",
  };

  const helpButton = {
    position: "absolute", top: isMobile ? 15 : 24, right: isMobile ? 15 : 24, padding: "8px 14px", borderRadius: 10, fontSize: 13, textDecoration: "none", color: "#7dd3fc", border: "1px solid #1e293b", background: "rgba(2,6,23,0.6)", zIndex: 15,
  };

  return (
    <main
      style={{
        minHeight: "100vh", position: "relative", padding: isMobile ? "20px 15px" : "40px", color: "white", fontFamily: "Inter, system-ui, sans-serif", backgroundColor: "#020617",
        backgroundImage: `
        ...
        backgroundSize: isMobile ? "auto, auto, 200% 200%, 200% 200%, 200% 200%, 420px auto" : "auto, auto, 100% 100%, 100% 100%, 100% 100%, 420px auto",
    ...
    >
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 20 }}>
          <div style={card}>
            ...
            {[["Monthly Income ($)", "income"], ["Fixed Expenses", "fixed"], ["Variable Expenses", "variable"]].map(([label, key]) => (
              <div key={key} style={{ marginBottom: 15 }}>
                <label style={{ fontSize: "14px" }}>{label}</label>
                <input type="number" value={data[key]} style={input} onChange={(e) => setData({ ...data, [key]: Number(e.target.value) })} />
              </div>
            ))}
          </div>
          ...
          <div style={card}>
            ...
            <ul style={{ paddingLeft: 20 }}>
              {insights.map((i, idx) => (<li key={idx} style={{ marginBottom: 12, fontSize: "14px" }}>{i}</li>))}
            </ul>
          </div>
        </div>
        ...
        <div id="pricing" style={{ marginTop: isMobile ? 40 : 60 }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 20, flexWrap: "wrap" }}>
            <div style={priceCard} onClick={() => handleCheckout("price_1SscYJDyLtejYlZiyDvhdaIx")}>
              <h3>1 Day · $9.99</h3>
              <small>Immediate clarity</small>
            </div>
            <div style={priceCard} onClick={() => handleCheckout("price_1SscaYDyLtejYlZiDjSeF5Wm")}>
              <h3>1 Week · $14.99</h3>
              <small>Behavior & patterns</small>
            </div>
            {/* MONTH PLAN - FIXED ID */}
            <div style={{...priceCard, border: "1px solid #10b981"}} onClick={() => handleCheckout("price_1SyaeRDyLtejYlZiWo76wuWO")}>
              <h3>1 Month · $49.99</h3>
      ...
      <style>{`
        ...
        @keyframes pulseSoft { 0% { opacity: 0.6; } 50% { opacity: 1; } 100% { opacity: 0.6; } }
      `}</style>
    </main>
  );
}
